...
class photo_control extends admin_control {
	...
	public function index() {
		...
		// 获取分类下拉框
		$cidhtml = $this->category->get_cidhtml_by_mid($this->_mid, $cid, '所有图集');
	...
	public function add() {
		...
		if(empty($_POST)) {
			...
			// 防止提交到其他模型的分类
			if($table != 'photo') $this->message(0, '分类ID非法！');
			...
			//该钩子用来扩展主表
			// hook admin_photo_control_add_content_after.php
			...
			// 写入内容数据表
			//该钩子用来扩展副表
			// hook admin_photo_control_add_content_data_after.php
			...
			//删除自动保存数据
			$this->kv->delete('auto_save_photo_uid_'.$uid);
			...
			$this->message(1, '发布完成'.$endstr,'index.php?u=photo-index');
		}
	}
	...
	public function edit() {
		...
		if(empty($_POST)) {
			...
			$this->message(1, '编辑完成'.$endstr,'index.php?u=photo-index');
		}
	}
	...
	public function batch_del() {
		...
		if(!empty($id_arr) && is_array($id_arr)) {
			$err_num = 0;
			foreach($id_arr as $v) {
				$err = $this->cms_content->xdelete('photo', $v);
				if($err) $err_num++;
			}
			
			if($err_num) {
				$this->message(0,$err_num.' 篇图集删除失败！');
			}else{
				$this->message(1,'删除成功！');
			}
		}else{
	...
	public function save_images() {
		...
		empty($images) && $this->message(0,'亲，您的图集忘上传图片了！');
