...
class category extends model {
	...
	public function get_cidhtml_by_mid($_mid, $cid = 0, $tips = '选择分类') {
	...
	public function category_url(&$cid, &$alias, $page = FALSE) {
		...
		if(empty($_ENV['_config']['molycms_parseurl'])) {
			return webdomain($this->cfg['webdomain']).$this->cfg['webdir'].'index.php?cate-index-cid-'.$cid.($page ? '-page-{page}' : '').$_ENV['_config']['url_suffix'];
		}else{
			if($page) {
				return webdomain($this->cfg['webdomain']).$this->cfg['webdir'].$alias.$this->cfg['link_cate_page_pre'].'{page}'.$this->cfg['link_cate_page_end'];
			}else{
				return webdomain($this->cfg['webdomain']).$this->cfg['webdir'].$alias.$this->cfg['link_cate_end'];
			}
		}
	}
}
