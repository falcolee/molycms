...
function kp_block_list($conf) {
	...
	$orderby = isset($conf['orderby']) && in_array($conf['orderby'], array('id', 'dateline','listorder','comments')) ? $conf['orderby'] : 'id';
	$orderway = isset($conf['orderway']) && $conf['orderway'] == 1 ? 1 : -1;
	$start = _int($conf, 'start');
	$limit = _int($conf, 'limit', 10);
	$more = _int($conf, 'more', 0);
	...
	if( $more ){
		$run->cms_content_data->table = 'cms_'.$table.'_data';
	}
	...
	foreach($list_arr as &$v) {
		if( $more ){
			$more_info = $run->cms_content_data->read($v['id']);
			if( is_array($more_info) && $more_info ){
				$v = array_merge($more_info,$v);
			}
		}
