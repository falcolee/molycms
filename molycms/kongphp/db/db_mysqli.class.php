...
class db_mysqli implements db_interface {
	...
	public function index_drop($table, $index) {
        ...
        public function drop_table($table) {
            return $this->query("DROP TABLE IF EXISTS $table",  $this->wlink);
        }
	 ...
	 * @return array
	 */        
        function check_fields($table, $array){
	...
	function get_fields($table) {
		$fields = array();
		return $this->fetch_first("SHOW COLUMNS FROM $table");
	}
	...
	public function query($sql, $link = NULL, $isthrow = TRUE) {
		...
		if(!$result && $isthrow) {
			$s = 'MySQL Query Error: <b>'.$sql.'</b>. '.mysqli_error();
	...
	function S($str, $quot="'") {
		return is_numeric($str) ? $str : $quot.mysqli_real_escape_string($str).$quot;
	}
