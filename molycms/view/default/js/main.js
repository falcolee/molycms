...
$(function() {
	...
	(function(){
		...
		var cont_v = cont.val();
		var maybe_author = $.cookie("maybe_comment_author");
		if(maybe_author) {
			var author_v = maybe_author;
		}else{
		...
		$("#ctf_form").submit(function(){
			...
			var code_v = $("#code").val();
			$.cookie("maybe_comment_author", author_v, {expires:3650}); // 写入cookie
			...
			if(!code_v) {
				$("#ctf_tips").html('<font color="red">请填写验证码！</font>');
				return false;
			}
			var _this = $(this);
